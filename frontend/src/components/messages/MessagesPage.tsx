...
export default function MessagesPage({ activeUserId }: { activeUserId?: string }) {
  ...
  return (
    <div className="p-3 md:p-6 max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-6 h-[calc(100vh-80px)]">
      {/* Conversations List */}
      <div
        className={`${
          activeUserId ? 'hidden lg:block' : 'block'
        } lg:col-span-1 neo-card p-4 space-y-3 overflow-y-auto h-full`}
      >
        ...
        {isLoading ? (
          ...
          conversations.map((c) => {
            ...
            // Determine other partner relative to logged in user
      ...
      <div
        className={`${
          activeUserId
            ? 'fixed inset-x-0 top-16 bottom-16 z-30 p-2 bg-black-midnight lg:static lg:p-0 lg:z-0 lg:bg-transparent lg:col-span-2'
